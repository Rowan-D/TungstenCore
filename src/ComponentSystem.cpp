...
namespace wCore
{
    ...
    SceneHandle ComponentSystem::CreateScene(std::string_view name)
    {
        ...
        if (m_sceneFreeList.Empty())
        {
            ...
            std::memset(m_freeLists + sceneStartFreeListIndex, 0, sizeof(wUtils::RelocatableFreeListHeader<ComponentIndex>) * m_currentComponentTypeCount);
    ...
    void ComponentSystem::ReserveComponents(ComponentTypeIndex componentTypeIndex, SceneIndex sceneIndex, wIndex minCapacity)
    {
        ...
        wIndex& capacity = m_indexes[sceneStartIndexIndex + m_currentComponentTypeCount + componentTypeIndex - 1];

        W_IF_UNLIKELY(type.pageSize)
        {
            const wIndex minPageCount = wUtils::IntDivCeil(minCapacity, type.pageSize);
            if (minCapacity > capacity * type.pageSize) {
    ...
    void ComponentSystem::ReallocateScenes(wIndex newCapacity)
    {
        ...
        offset = wUtils::AlignUp(offset, alignof(wUtils::RelocatableFreeListHeader<ComponentIndex>));
        const std::size_t freeListOffset = offset;
        offset += m_currentComponentTypeCount * newCapacity * sizeof(wUtils::RelocatableFreeListHeader<ComponentIndex>);

        offset = wUtils::AlignUp(offset, alignof(SceneData));
        const std::size_t sceneDataOffset = offset;
        offset += newCapacity * sizeof(SceneData);

        constexpr std::size_t alignment = std::max(std::max(std::max(alignof(void*), alignof(SceneGeneration)), alignof(wIndex)), std::max(alignof(wUtils::RelocatableFreeListHeader<ComponentIndex>), alignof(SceneData)));
        if (m_scenes)
        {
            ...
            wUtils::RelocatableFreeListHeader<ComponentIndex>* newFreeLists = reinterpret_cast<wUtils::RelocatableFreeListHeader<ComponentIndex>*>(newScenes + freeListOffset);
            ...
            if (m_sceneSlotCount)
            {
                ...
                std::memcpy(newFreeLists, m_freeLists, m_sceneSlotCount * sizeof(wUtils::RelocatableFreeListHeader<ComponentIndex>));
        ...
        m_freeLists = reinterpret_cast<wUtils::RelocatableFreeListHeader<ComponentIndex>*>(m_scenes + freeListOffset);
